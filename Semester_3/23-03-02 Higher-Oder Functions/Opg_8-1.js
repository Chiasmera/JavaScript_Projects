...
console.log(persons)
console.log(`Person med tlf 23801777: ${persons
    .filter( (element) => element.phone === 23801777 )[0].name}`)

console.log(`mindste alder: ${persons
                                  .reduce( (min, user) => user.age < min ? user.age : min, persons[0].age )}`)


persons
     .forEach( (element, index, array) => element.id = index+1 )
console.log(`alle personer tildelt et id:`)
persons
    .forEach( (el) => console.log(el.name+': id='+el.id))

console.log(`Array indeholder navne: `+ persons
                                                .map( (person) => person.name) 
                                                .sort()
                                                .reduce( (acc, name) => acc+name+", "  , ""))


let under30Array = persons
